...
> **Abstract:** Efficient spatiotemporal IoT data indexing is crucial for real-time analytics and query processing. However, conventional tree-based spatiotemporal indexes struggle to balance update efficiency and query performance, especially under high-frequency data streams. Frequent insertions, deletions, and updates cause excessive node splits and rebalancing, increasing computational overhead. Moreover, the cache-unfriendly nature of existing indexing structures results in frequent cache misses, leading to excessive memory access and degraded query performance. To address these challenges, we propose TST, a novel trie-based spatiotemporal index leveraging temporal prefixes. TST introduces: (1) an encoding scheme that preserves 3D spatiotemporal locality while compressing it into a 1D representation, (2) constant time data insertion and deletion using bitwise operations, and (3) a cache-friendly design with memory-aligned node sizes and an array-based structure for efficient access. Extensive experiments on three real-world datasets demonstrate that TST reduces reorganization overhead by up to 40×, accelerates index construction by 24×, and improves cache hit rate by 8×, resulting in up to 1219× faster query response times compared to existing methods.
...
<p align="center">
  ...
  <br>
  <b>Figure 1</b>: Overall structure of TST, with minimum time resolution <i>hour</i> and an S2 cell encoding at <i>level 20</i>: "<i>d</i>" indicates the depth of TST, and the route colored in red exemplifies a search path via the array index.
</p>
...
- Code Author : Jooyoung Yoon

- Contact : aloneghost@knu.ac.kr
